    ...
    with torch.no_grad():
        for i_iter_val, ( input_occs, target_occs, metas) in enumerate(val_dataset_loader):
            ...
            if i_iter_val < start_frame:
                continue'''
            #print("dicttttttttttttttttttttt",input_dict)
            #print("metasssssssss",metas[0]["rel_poses"])
            gggg = metas[0]["gt_mode"]
            # Handle both 1D and 2D arrays
            if gggg.ndim == 1:
                gggg = np.delete(gggg, -1)  # 1D array: delete last element
            else:
                gggg = np.delete(gggg, -1, axis=1)  # 2D array: delete last column
            print(gggg.dtype)
            print("gggg",gggg.shape)
            gggg = gggg.astype(np.float32)
            save_path = os.path.join(args.work_dir, "step32-2/gt_mode/")
            os.makedirs(save_path, exist_ok=True)
            np.save(os.path.join(save_path, f"i_iter_{i_iter_val}"), gggg) 
            ...
            #print("mressssssssssssssssssssssssssssssssss",result)
            logits = result['logits'] # 1 12 200 200 16
            ...
            token_save_path = os.path.join(args.work_dir, "step32-2/token/")
            os.makedirs(token_save_path, exist_ok=True)
            np.save(os.path.join(token_save_path, f"i_iter_{i_iter_val}"), middd) 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--scene-idx', nargs='+', type=int, default=[1,2,3,5,200,1108,1109,1110])
