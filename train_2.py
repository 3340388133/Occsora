...
def bbox_corners_2d(data):
    #
    x, y, z, x_size, y_size, z_size, yaw = data[:, 0], data[:, 1], data[:, 2], data[:, 3], data[:, 4], data[:, 5], data[:, 6]
    
    #
    corners = np.array([[-x_size / 2, -y_size / 2, -z_size / 2],  #
    ...
    rotated_corners = np.dot(corners[:, [0, 1]], rot_matrix.T)
    corners[:, [0, 1]] = rotated_corners
...
class CustomDataset(Dataset):
    def __init__(self, folder_a, folder_b):
        self.files_a = sorted([os.path.join(folder_a, f) for f in os.listdir(folder_a) if f.endswith('.npy')])
        self.files_b = sorted([os.path.join(folder_b, f) for f in os.listdir(folder_b) if f.endswith('.npy')])
...
def main(args):
    ...
    local_vae_path = f"/root/autodl-tmp/models/AI-ModelScope/sd-vae-ft-{args.vae}"
    ...
    file1_path = '/root/autodl-tmp/OccSora_output/vqvae/step32-2/token/'
    file2_path = '/root/autodl-tmp/OccSora_output/vqvae/step32-2/gt_mode/'
   ...
   # Initial state
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, y   in loader:
            ...
            x = x /10
            #y = y /10
            print("beforexxx",x.shape)
            #print("beforeyyy",y.shape)
